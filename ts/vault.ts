...
import {
  randomBytes, createCipheriv, createDecipheriv, scryptSync, randomUUID,
} from "crypto";
import { readFileSync, writeFileSync, existsSync } from "fs";
import { loadConfig, appendAudit } from "./state.js";
import type { ActionPlan, Action } from "./types.js";
...
class Vault {
  ...
  load(): void {
    ...
    try {
      ...
      throw new Error(
        `Failed to decrypt vault. Wrong password or corrupted file. Error: ${err}`
      );
    }
  }
  ...
  listPlans(): Array<{ id: string; name: string; actionCount: number; updatedAt: string }> {
    this.ensureLoaded();
  ...
  getPlan(id: string): ActionPlan | undefined {
    this.ensureLoaded();
  ...
  createPlan(name: string, actions: Action[]): ActionPlan {
    this.ensureLoaded();
  ...
  updatePlan(id: string, updates: Partial<Pick<ActionPlan, "name" | "actions">>): ActionPlan {
    this.ensureLoaded();

    const index = this.plans.findIndex((p) => p.id === id);
    if (index === -1) throw new Error(`Plan not found: ${id}`);

    const plan = this.plans[index];
    if (updates.name) plan.name = updates.name;
    if (updates.actions) plan.actions = updates.actions;
  ...
  exportBackup(backupPassword: string): Buffer {
    this.ensureLoaded();
    const json = JSON.stringify(this.plans, null, 2);
  ...
  importBackup(data: Buffer, backupPassword: string): void {
    const json = decrypt(data, backupPassword);
    const plans = JSON.parse(json) as ActionPlan[];
...
function main(): void {
  ...
  if (pwIdx !== -1) {
    ...
    if (valIdx !== -1) args.splice(valIdx, 1);
  }
  ...
  switch (command) {
    ...
    case "get": {
      const id = args[1];
      if (!id) { fail("Usage: vault.ts get <plan-id>"); return; }
      const vault = new Vault(password);
      vault.load();
      const plan = vault.getPlan(id);
      if (!plan) { fail(`Plan not found: ${id}`); return; }
      output(plan);
      break;
    }
    ...
    case "create": {
      ...
      if (!planJson) { fail("Usage: vault.ts create '<json>'"); return; }
      const { name, actions } = JSON.parse(planJson) as { name: string; actions: Action[] };
      if (!name || !actions) { fail("JSON must have 'name' and 'actions' fields"); return; }
      const vault = new Vault(password);
      vault.load();
    ...
    case "update": {
      ...
      if (!id || !updatesJson) { fail("Usage: vault.ts update <id> '<json>'"); return; }
      const updates = JSON.parse(updatesJson) as Partial<Pick<ActionPlan, "name" | "actions">>;
      const vault = new Vault(password);
      vault.load();
    ...
    case "delete": {
      const id = args[1];
      if (!id) { fail("Usage: vault.ts delete <plan-id>"); return; }
      const vault = new Vault(password);
      vault.load();
    ...
    case "import": {
      ...
      if (!inFile) { fail("Usage: vault.ts import <file> [backup-password]"); return; }
