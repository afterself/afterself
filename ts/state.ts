...
import type { AfterselfState, AfterselfConfig, AuditEntry } from "./types.js";
...
function defaultState(): AfterselfState {
  return {
...
export function loadState(): AfterselfState {
  ensureDir();
...
export function saveState(state: AfterselfState): void {
  ensureDir();
  writeFileSync(STATE_FILE, JSON.stringify(state, null, 2), { mode: 0o600 });
}

export function updateState(
  updater: (state: AfterselfState) => AfterselfState
): AfterselfState {
...
function isOverdue(): { overdue: boolean; elapsed: string | null; interval: string } {
...
function isWarningExpired(): { expired: boolean; elapsed: string | null; warningPeriod: string } {
  const state = loadState();
  const config = loadConfig();
...
function escalationStatus(): {
  state: string;
  responses: AfterselfState["escalationResponses"];
  totalContacts: number;
  aliveCount: number;
  absentCount: number;
  decision: "stand_down" | "trigger" | "waiting";
} {
  const state = loadState();
  const config = loadConfig();
  ...
  let decision: "stand_down" | "trigger" | "waiting" = "waiting";
  if (aliveCount > 0) {
    decision = "stand_down";
...
function main(): void {
  ...
  switch (command) {
    ...
    case "audit": {
      const type = args[1] as AuditEntry["type"];
      const action = args[2];
    ...
    case "record-escalation-response": {
      ...
      const response = args[2] as "confirmed_alive" | "confirmed_absent" | "no_response";
