...
import type { PersonaProfile, SampleMessage, Channel } from "./types.js";
...
export function loadPersona(): PersonaProfile {
...
export function savePersona(persona: PersonaProfile): void {
...
function analyzeWritingStyle(
  messages: string[],
  existing: PersonaProfile["writingStyle"]
): PersonaProfile["writingStyle"] {
  const avgLen = messages.reduce((sum, m) => sum + m.length, 0) / messages.length;
  const averageMessageLength: "short" | "medium" | "long" =
    avgLen < 50 ? "short" : avgLen < 200 ? "medium" : "long";
  ...
  const casualCount = messages.filter((m) =>
    casualIndicators.some((ind) => m.toLowerCase().includes(ind))
  ).length;
  const casualRatio = casualCount / messages.length;
  const formality: "casual" | "mixed" | "formal" =
    casualRatio > 0.3 ? "casual" : casualRatio > 0.1 ? "mixed" : "formal";
  ...
  if (exclamationRate > 0.4) punctuationStyle = "enthusiastic (lots of !)";
  else if (ellipsisRate > 0.2) punctuationStyle = "trailing (uses ... often)";
  else if (questionRate > 0.3) punctuationStyle = "inquisitive (lots of ?)";
...
function extractTopics(messages: string[], existing: string[]): string[] {
...
function selectSampleMessages(
  ...
  const scored = messages.map((m) => ({
    message: m,
    score: personalityIndicators.reduce(
      (score, indicator) =>
        score + (m.content.toLowerCase().includes(indicator) ? 1 : 0),
      0
    ) + (m.content.length > 30 && m.content.length < 300 ? 2 : 0),
  }));
  ...
  const deduped = merged.filter((m) => {
    if (seen.has(m.message)) return false;
    seen.add(m.message);
    return true;
  });
...
function retrieveRelevant(
  query: string,
  samples: PersonaProfile["sampleMessages"],
  limit: number
): PersonaProfile["sampleMessages"] {
...
function main(): void {
  ...
  switch (command) {
    ...
    case "analyze": {
      ...
      if (!inputFile) { fail("Usage: persona.ts analyze --input <file.json>"); return; }
    ...
    case "retrieve": {
      ...
      if (!query) { fail("Usage: persona.ts retrieve --query \"text\""); return; }
    ...
    case "set-name": {
      const name = args[1];
      if (!name) { fail("Usage: persona.ts set-name <name>"); return; }
      const persona = loadPersona();
      persona.name = name;
      savePersona(persona);
    ...
    case "set-humor": {
      const humor = args[1] as PersonaProfile["writingStyle"]["humor"];
    ...
    case "add-blocked-topic": {
      const topic = args[1];
      if (!topic) { fail("Usage: persona.ts add-blocked-topic <topic>"); return; }
      const persona = loadPersona();
    ...
    case "remove-blocked-topic": {
      const topic = args[1];
      if (!topic) { fail("Usage: persona.ts remove-blocked-topic <topic>"); return; }
      const persona = loadPersona();
