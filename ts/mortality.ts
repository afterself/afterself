...
import { loadConfig, saveConfig, loadState, updateState, appendAudit } from "./state.js";
...
function getKeypairPath(): string {
  ...
  if (!path) {
    throw new Error(
      "No keypair configured. Run 'create-wallet' or set mortalityPool.keypairPath in config."
    );
  }
  return path;
}
...
async function transferToPool(): Promise<{
  success: boolean;
  txSignature: string;
  amount: number;
}> {
  ...
  const poolTokenAccount = await getOrCreateAssociatedTokenAccount(
    connection,
    keypair, // payer (user pays for pool ATA creation if needed)
    tokenMint,
    poolWallet
  );
  ...
  const transferIx = createTransferInstruction(
    ...
    walletPubkey,
    BigInt(userTokenAccount.amount),
    [],
    TOKEN_PROGRAM_ID
  );
...
async function poolBalance(): Promise<{
  poolWallet: string;
  balance: number;
  tokenMint: string;
}> {
  ...
  let balance = 0;
  try {
    const poolTokenAddress = await getAssociatedTokenAddress(
      tokenMint,
      poolWallet
    );
